...
// Google API Key - check environment or use a known key from scripts
const GOOGLE_API_KEY = process.env.NEXT_PUBLIC_GOOGLE_PLACES_API_KEY || 
                       process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY ||
                       process.env.GOOGLE_PLACES_API_KEY ||
                       process.env.GOOGLE_MAPS_API_KEY ||
                       'AIzaSyCOYUi9A598QJGWk7hZK0_6_ojdvSNhZhw'; // Fallback from fetchPlaceDetails.js
...
async function searchPlace(venueName, location, address) {
  try {
    const query = `${venueName}, ${location || 'Slough, UK'}`;
    ...
    if (data.status === 'OK' && data.results && data.results.length > 0) {
      // Find best match (exact name match preferred, then Slough/Southall in address)
      const exactMatch = data.results.find(r => {
        const nameMatch = r.name.toLowerCase() === venueName.toLowerCase();
        const locationMatch = (r.formatted_address || '').toLowerCase().includes(location.toLowerCase());
      ...
      const nameMatch = data.results.find(r => 
...
async function getPlaceDetails(placeId) {
  try {
    ...
    const params = new URLSearchParams({
      place_id: placeId,
      fields: 'place_id,name,formatted_address,photos,geometry,rating,user_ratings_total',
      key: GOOGLE_API_KEY,
    });
...
async function updateVenues() {
  ...
  for (const venue of venuesToUpdate) {
    try {
      ...
      console.log(`   Location: ${venue.area || venue.borough || 'N/A'}`);
      ...
      console.log(`      Address: ${searchResult.formatted_address || 'N/A'}`);
  ...
  if (updated > 0) {
    ...
    // Save update log
    const logPath = path.join(__dirname, '../reports/venue-placeid-updates.json');
    fs.writeFileSync(logPath, JSON.stringify({
      ...
      updates: updates.map(u => ({
        name: u.venue.name,
        original_place_id: u.originalPlaceId,
        new_place_id: u.newPlaceId,
        has_photos: u.details.photos ? u.details.photos.length > 0 : false
      }))
    }, null, 2));
    console.log(`📝 Update log saved to: ${path.basename(logPath)}`);
  }
