...
async function verifyImages() {
  ...
  const publicPath = path.join(__dirname, '../public/venues.json');
  const dataPath = path.join(__dirname, '../data/venues.json');
  const venuesPath = fs.existsSync(publicPath) ? publicPath : dataPath;
  ...
  console.log(`📂 Using: ${path.basename(venuesPath)}`);
  ...
  // Load venue data (already determined above)
