...
const IMAGE_CONFIG = {
  ...
  quality: 85,
  maxApiCalls: 1000, // Free tier limit
  delayBetweenCalls: 100 // ms delay to respect rate limits
};

// Well-known restaurant image mappings (priority images)
...
const CUISINE_IMAGE_MAP = {
  ...
  'spanish': 'https://images.unsplash.com/photo-1568901346375-23c9450c58cd?w=1600&h=1200&fit=crop&crop=center&q=85',
  'american': 'https://images.unsplash.com/photo-1551782450-a2132b4ba21d?w=1600&h=1200&fit=crop&crop=center&q=85',
  'seafood': 'https://images.unsplash.com/photo-1563379091339-03246963d4d8?w=1600&h=1200&fit=crop&crop=center&q=85',
  'steakhouse': 'https://images.unsplash.com/photo-1555939594-58d7cb561ad1?w=1600&h=1200&fit=crop&crop=center&q=85'
};
...
async function fetchGooglePlacesImage(placeId, venueName) {
  try {
    ...
    if (data.status === 'OK' && data.result.photos && data.result.photos.length > 0) {
      ...
      const imageUrl = `https://maps.googleapis.com/maps/api/place/photo?maxwidth=${IMAGE_CONFIG.maxWidth}&photoreference=${photo.photo_reference}&key=${GOOGLE_PLACES_API_KEY}`;
      
      return {
        success: true,
        imageUrl: imageUrl,
        source: 'google_places',
        width: photo.width || IMAGE_CONFIG.maxWidth,
        height: photo.height || IMAGE_CONFIG.maxHeight
      };
    }
...
async function findPlaceId(venueName, address) {
  try {
    ...
    const searchUrl = `https://maps.googleapis.com/maps/api/place/findplacefromtext/json?input=${encodeURIComponent(query)}&inputtype=textquery&fields=place_id,name&key=${GOOGLE_PLACES_API_KEY}`;
...
async function buildComprehensiveImagePipeline() {
  console.log('🚀 BUILDING COMPREHENSIVE IMAGE PIPELINE WITH GOOGLE PLACES API...\n');
  
  // 1. Validate API key
  const apiValid = await validateGoogleAPI();
  if (!apiValid) {
    console.log('❌ Google Places API validation failed. Exiting.');
    return;
  }
  ...
  console.log(`Free tier limit: ${IMAGE_CONFIG.maxApiCalls} calls/day`);
  console.log(`Within free tier: ${estimatedApiCalls <= IMAGE_CONFIG.maxApiCalls ? '✅ YES' : '❌ NO'}`);
  ...
  console.log('🔄 PROCEEDING WITH GOOGLE PLACES API CALLS...');
  console.log(`Processing ${estimatedApiCalls} venues with API calls...\n`);
  
  // 6. Update images with API calls
  let updatedCount = 0;
  let apiCallsUsed = 0;
  const updatedVenues = [];
  ...
  for (let i = 0; i < venuesData.venues.length; i++) {
    ...
    let imageSource = getImageSource(currentImage);
    let imageQuality = quality.quality;
    ...
    if (quality.quality === 'low_res' || quality.quality === 'generic' || quality.quality === 'missing') {
      ...
      if (restaurantImage) {
        ...
        imageQuality = 'restaurant_website';
      } else if (apiCallsUsed < IMAGE_CONFIG.maxApiCalls) {
        // Try Google Places API
        console.log(`🔍 Fetching image for ${venue.name}...`);
        ...
        if (placeId) {
          ...
          if (imageResult.success) {
            ...
            analysis.totalCost += 0.017;
          } else {
            // Fallback to cuisine-specific image
        ...
        await sleep(IMAGE_CONFIG.delayBetweenCalls);
      } else {
  ...
  console.log(`\n✅ COMPREHENSIVE IMAGE PIPELINE COMPLETE!`);
  ...
  const report = {
    ...
    imageSources: {
      restaurantWebsite: updatedVenues.filter(v => v.image_source === 'restaurant_website').length,
      googlePlaces: updatedVenues.filter(v => v.image_source === 'google_places').length,
      unsplash: updatedVenues.filter(v => v.image_source === 'unsplash').length
    },
    qualityBreakdown: {
      highQuality: updatedVenues.filter(v => v.image_quality === 'high_res' || v.image_quality === 'restaurant_website').length,
      lowQuality: updatedVenues.filter(v => v.image_quality === 'low_res').length,
      missing: updatedVenues.filter(v => v.image_quality === 'missing').length
    }
  };
  
  const reportPath = path.join(__dirname, '../reports/images.md');
  fs.writeFileSync(reportPath, `# Comprehensive Image Pipeline Report
...
- **Google Places API**: ${report.imageSources.googlePlaces}
...
- **Max API Calls**: ${IMAGE_CONFIG.maxApiCalls}
...
buildComprehensiveImagePipeline().catch(console.error);
