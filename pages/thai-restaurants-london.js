...
export default function ThaiRestaurants({ venues }) {
  ...
  return (
    <>
      <Head>
        <title>Best Thai Restaurants in London 2025 | Authentic Thai Cuisine | The Best in London</title>
        <meta name="description" content={`Discover ${venues.length} top-rated Thai restaurants in London. Pad Thai, curries, and authentic flavors.`} />
        <link rel="canonical" href="https://thebestinlondon.co.uk/thai-restaurants-london" />
        ...
        <section style={{
          ...
          <div style={{ maxWidth: '900px', width: '100%', position: 'relative', zIndex: 1 }}>
            ...
            <p style={{ fontSize: 'clamp(16px, 2vw, 19px)', color: 'rgba(250, 250, 250, 0.95)', marginBottom: theme.spacing['2xl'], lineHeight: 1.7, textShadow: '0 2px 12px rgba(0,0,0,0.6)' }}>
              From Pad Thai to Green Curry — {venues.length} authentic Thai restaurants
            </p>
          ...
          <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 16px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
              <div style={{ color: theme.colors.text.secondary, fontSize: '14px' }}>
                Showing <strong style={{ color: theme.colors.text.primary }}>{sorted.length}</strong> Thai restaurants
              </div>
...
export async function getStaticProps() {
  ...
  try {
    ...
    const venues = allVenues.filter(v => v && (v.cuisines?.some(c => c.toLowerCase().includes('thai')) || v.name?.toLowerCase().includes('thai'))).sort((a, b) => (b.rating || 0) - (a.rating || 0));
    return { props: { venues }};
  } catch (error) {
    return { props: { venues: [] } };
  }
