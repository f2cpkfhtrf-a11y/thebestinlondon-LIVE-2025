...
import { LONDON_STATIONS, getVenuesNearStation } from '../../../utils/halalStations';
  ...
  return (
    <>
      ...
      <div style={{ minHeight: '100vh', background: theme.colors.background.primary }}>
        ...
        {/* Breadcrumbs */}
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: `${theme.spacing.md} ${theme.spacing.lg}` }}>
                ...
                {radiusOptions.map(option => (
                  <button
                    ...
                    style={{
                      ...
                      borderRadius: '8px',
                      color: selectedRadius === option.value ? theme.colors.background.primary : theme.colors.text.primary,
                      fontSize: '0.9375rem',
                      fontWeight: 600,
                      cursor: 'pointer',
                      transition: 'all 0.2s'
                    ...
                    <Link href={`/restaurant/${venue.slug}`} style={{ textDecoration: 'none', color: 'inherit' }}>
                      ...
                      <div style={{ padding: theme.spacing.lg }}>
                        ...
                        <div style={{ display: 'flex', gap: theme.spacing.sm, marginBottom: theme.spacing.sm, flexWrap: 'wrap' }}>
                          <BestOfLondonBadge score={venue.bestOfLondonScore} />
                          {venue.rating && (
                            <div style={{
        ...
        {/* Nearby Stations */}
        {station.nearbyStations?.length > 0 && (
          <section style={{
            ...
            <div style={{
              ...
              {station.nearbyStations.map(nearby => (
                <Link key={nearby.slug} href={`/halal/near-stations/${nearby.slug}`} style={{
                  display: 'block',
                  padding: theme.spacing.lg,
                  background: `${theme.colors.text.primary}05`,
                  border: `1px solid ${theme.colors.border.subtle}`,
                  ...
                  <div style={{ fontSize: '0.875rem', color: theme.colors.text.secondary }}>
                    {nearby.distance.toFixed(1)}km away • {nearby.count} venues
                  </div>
                </Link>
              ))}
            </div>
...
export async function getStaticProps({ params }) {
  ...
  const nearbyStations = LONDON_STATIONS
    ...
    .map(s => {
      ...
      return { ...s, distance, count: getVenuesNearStation(s, venuesData, 0.6).length };
    })
  ...
  return {
    props: {
      station: { ...station, nearbyStations },
      venuesByRadius
    }
  };
}
