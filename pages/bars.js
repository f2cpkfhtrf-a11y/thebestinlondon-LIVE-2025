...
export async function getStaticProps() {
  ...
  try {
    ...
    const barVenues = (data.venues || data)
      .map(enhanceVenueData)
      .filter(v => {
        if (!v) return false;
        const types = (v.types || []).map(t => t.toLowerCase()).join(' ');
        return types.includes('bar') || types.includes('pub') || types.includes('cocktail');
      })
      .sort((a, b) => (b.rating || 0) - (a.rating || 0));
    
    return { props: { venues: barVenues }};
  } catch (error) {
...
export default function BarsLondon({ venues }) {
        ...
        <header style={{ padding: '80px 20px', background: `linear-gradient(135deg, ${theme.colors.bg.primary} 0%, ${theme.colors.bg.elevated} 100%)` }}>
          <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
            ...
            <p style={{ fontSize: '18px', color: theme.colors.text.secondary, maxWidth: '800px' }}>
              Discover {venues.length}+ top-rated bars across London. From cocktail lounges to traditional pubs and rooftop terraces.
            </p>
          </div>
        </header>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '32px' }}>
            {filtered.length === 0 ? (
              <div style={{ gridColumn: '1 / -1', textAlign: 'center', padding: '80px 20px', color: theme.colors.text.secondary }}>
                <p style={{ fontSize: '18px' }}>No bars found. Check back soon!</p>
