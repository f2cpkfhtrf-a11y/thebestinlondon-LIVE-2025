...
export default function ItalianRestaurants({ venues }) {
  ...
  return (
    <>
      <Head>
        <title>Best Italian Restaurants in London 2025 | Pizza & Pasta | The Best in London</title>
        <meta name="description" content={`Discover ${venues.length} authentic Italian restaurants in London. Wood-fired pizza, fresh pasta, risotto.`} />
        ...
        <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
          ...
          "description": `Discover ${venues.length} authentic Italian restaurants in London`,
          "url": "https://thebestinlondon.co.uk/italian-restaurants-london"
        }) }} />
        ...
        <section style={{
          ...
          <div style={{ maxWidth: '900px', width: '100%', position: 'relative', zIndex: 1 }}>
            ...
            <p style={{ fontSize: 'clamp(16px, 2vw, 19px)', color: 'rgba(250, 250, 250, 0.95)', marginBottom: theme.spacing['2xl'], lineHeight: 1.7, textShadow: '0 2px 12px rgba(0,0,0,0.6)' }}>
              From wood-fired pizza to handmade pasta — {venues.length} authentic Italian restaurants
            </p>
          ...
          <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 16px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
              <div style={{ color: theme.colors.text.secondary, fontSize: '14px' }}>
                Showing <strong style={{ color: theme.colors.text.primary }}>{sorted.length}</strong> Italian restaurants
              </div>
                  ...
                  <article style={{ background: theme.colors.bg.elevated, border: `1px solid ${theme.colors.border.subtle}`, borderRadius: '16px', overflow: 'hidden', transition: 'all 0.3s ease', cursor: 'pointer', height: '100%' }} className="venue-card">
                    ...
                    <div style={{ padding: 'clamp(16px, 3vw, 20px)' }}>
                      ...
                      <div style={{ display: 'flex', gap: '12px', marginBottom: '12px' }}>
                        <span style={{ color: theme.colors.text.secondary, fontSize: '14px' }}>🍝 Italian</span>
...
export async function getStaticProps() {
  ...
  try {
    ...
    const venues = allVenues.filter(v => v && (v.cuisines?.some(c => c.toLowerCase().includes('italian')) || v.name?.toLowerCase().includes('italian') || v.name?.toLowerCase().includes('pizza') || v.name?.toLowerCase().includes('pasta'))).sort((a, b) => (b.rating || 0) - (a.rating || 0));
    return { props: { venues }};
  } catch (error) {
    return { props: { venues: [] } };
  }
