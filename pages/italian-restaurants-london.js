// ITALIAN RESTAURANTS - AUTO-GENERATED WITH THEME
  ...
  try {
    ...
    const venues = allVenues
      .filter(v => {
        ...
        return name.includes('italian') || types.includes('italian') || name.includes('pizza') || name.includes('pasta') || types.includes('pizza');
      })
      .map(v => {
...
export default function ItalianRestaurants({ venues }) {
  ...
  return (
    <>
      <Head>
        <title>Best Italian Restaurants London 2025 | Pizza & Pasta | The Best in London</title>
        <meta name="description" content={`Discover ${venues.length} authentic Italian restaurants in London. Wood-fired pizza, fresh pasta, risotto. Real photos & reviews.`} />
        <link rel="canonical" href="https://thebestinlondon.co.uk/italian-restaurants-london" />
      </Head>
      ...
      <div style={{ minHeight: '100vh', background: '#fafafa' }}>
        <header style={{ 
          ...
          <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
            ...
            <p style={{ fontSize: 'clamp(16px, 2.5vw, 20px)', opacity: 0.95, margin: '0 0 12px 0' }}>
              {venues.length} authentic Italian restaurants • Pizza • Pasta • Risotto
            </p>
        ...
        <div style={{ maxWidth: '1400px', margin: '-30px auto 40px', padding: '0 20px', position: 'relative', zIndex: 10 }}>
          <div style={{ background: 'white', borderRadius: '16px', padding: '24px', boxShadow: '0 10px 40px rgba(0,0,0,0.12)' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr auto auto auto', gap: '12px' }}>
              <input type="text" placeholder="Search Italian restaurants..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
              ...
              <button onClick={() => setShowComparison(!showComparison)}
                style={{
                  padding: '14px 20px',
                  background: showComparison ? '#16a34a' : 'white',
                  color: showComparison ? 'white' : '#16a34a',
        ...
        <main style={{ maxWidth: '1400px', margin: '0 auto', padding: '0 20px 60px' }}>
          ...
          <p style={{ fontSize: '16px', color: '#6b7280', marginBottom: '24px' }}>
            <strong style={{ color: '#16a34a' }}>{filtered.length}</strong> of {venues.length} restaurants
          </p>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(380px, 1fr))', gap: '24px' }}>
            {filtered.map((venue, idx) => (
              ...
              onMouseLeave={(e) => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = '0 2px 12px rgba(0,0,0,0.08)'; }}>
                <div style={{ height: '220px', position: 'relative', background: '#16a34a' }}>
                  <img src="https://images.unsplash.com/photo-1498579150354-977475b7ea0b?w=800&q=80" alt={venue.name}
                  ...
                  {idx < 3 && (
                    <div style={{
                      ...
                      fontWeight: 'bold', fontSize: '13px', boxShadow: '0 4px 12px rgba(0,0,0,0.3)'
                    }}>#{idx + 1}</div>
                  )}
                  {venue.area && (
                    <div style={{
                ...
                <div style={{ padding: '24px' }}>
                  ...
                  {venue.priceEstimate && (
                    <div style={{ marginBottom: '18px', padding: '10px 12px', background: '#f0fdf4', borderRadius: '10px',
                      display: 'flex', justifyContent: 'space-between' }}>
                      <span style={{ color: '#15803d', fontWeight: '700', fontSize: '15px' }}>💰 {venue.priceEstimate}pp</span>
                  ...
                  <button onClick={() => window.open(`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(venue.name + ' ' + venue.vicinity)}`, '_blank')}
                    style={{
                      ...
                      fontWeight: '600', border: 'none', cursor: 'pointer', boxShadow: '0 2px 8px rgba(22, 163, 74, 0.3)'
                    }}>
                    View Details →
                  </button>
                </div>
