...
export default function HalalRestaurants({ venues }) {
  ...
  return (
    <>
      <Head>
        <title>Best Halal Restaurants in London 2025 | Certified Halal Dining | The Best in London</title>
        <meta name="description" content={`Discover ${venues.length} top-rated halal restaurants in London. Certified halal cuisine across all cuisines.`} />
      ...
      <div style={{ minHeight: '100vh', background: theme.colors.bg.primary, color: theme.colors.text.primary }}>
        ...
        <section style={{
          ...
          background: `linear-gradient(to bottom, rgba(11,11,11,0.5), rgba(11,11,11,0.75)), url('https://images.unsplash.com/photo-1555939594-58d7cb561ad1?w=2400&q=90')`,
          ...
          <div style={{ maxWidth: '900px', width: '100%', position: 'relative', zIndex: 1 }}>
            ...
            <p style={{ fontSize: 'clamp(16px, 2vw, 19px)', color: 'rgba(250, 250, 250, 0.95)', marginBottom: theme.spacing['2xl'], lineHeight: 1.7, textShadow: '0 2px 12px rgba(0,0,0,0.6)' }}>
              Certified halal dining — {venues.length} halal restaurants
            </p>
          ...
          <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 16px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
              <div style={{ color: theme.colors.text.secondary, fontSize: '14px' }}>
                Showing <strong style={{ color: theme.colors.text.primary }}>{sorted.length}</strong> halal restaurants
              </div>
...
export async function getStaticProps() {
  ...
  try {
    ...
    const venues = allVenues.filter(v => v && (v.dietaryTags?.some(t => t.toLowerCase().includes('halal')) || v.name?.toLowerCase().includes('halal'))).sort((a, b) => (b.rating || 0) - (a.rating || 0));
    return { props: { venues }};
  } catch (error) {
    return { props: { venues: [] } };
  }
