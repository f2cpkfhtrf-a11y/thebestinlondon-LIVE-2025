...
export default function TurkishRestaurants({ venues }) {
  ...
  return (
    <>
      <Head>
        <title>Best Turkish Restaurants in London 2025 | Authentic Turkish Cuisine | The Best in London</title>
        <meta name="description" content={`Discover ${venues.length} top-rated Turkish restaurants in London. Kebabs, mezze, and authentic flavors.`} />
        <link rel="canonical" href="https://thebestinlondon.co.uk/turkish-restaurants-london" />
        ...
        <section style={{
          ...
          <div style={{ maxWidth: '900px', width: '100%', position: 'relative', zIndex: 1 }}>
            ...
            <p style={{ fontSize: 'clamp(16px, 2vw, 19px)', color: 'rgba(250, 250, 250, 0.95)', marginBottom: theme.spacing['2xl'], lineHeight: 1.7, textShadow: '0 2px 12px rgba(0,0,0,0.6)' }}>
              From Kebabs to Mezze — {venues.length} authentic Turkish restaurants
            </p>
          ...
          <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 16px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
              <div style={{ color: theme.colors.text.secondary, fontSize: '14px' }}>
                Showing <strong style={{ color: theme.colors.text.primary }}>{sorted.length}</strong> Turkish restaurants
              </div>
...
export async function getStaticProps() {
  ...
  try {
    ...
    const venues = allVenues.filter(v => v && (v.cuisines?.some(c => c.toLowerCase().includes('turkish')) || v.name?.toLowerCase().includes('turkish'))).sort((a, b) => (b.rating || 0) - (a.rating || 0));
    return { props: { venues }};
  } catch (error) {
    return { props: { venues: [] } };
  }
