...
export default function VegetarianRestaurants({ venues }) {
  ...
  return (
    <>
      <Head>
        <title>Best Vegetarian Restaurants in London 2025 | Meat-Free Dining | The Best in London</title>
        <meta name="description" content={`Discover ${venues.length} top-rated vegetarian restaurants in London. Delicious meat-free cuisine.`} />
      ...
      <div style={{ minHeight: '100vh', background: theme.colors.bg.primary, color: theme.colors.text.primary }}>
        ...
        <section style={{
          ...
          background: `linear-gradient(to bottom, rgba(11,11,11,0.5), rgba(11,11,11,0.75)), url('https://images.unsplash.com/photo-1512621776951-a57141f2eefd?w=2400&q=90')`,
          ...
          <div style={{ maxWidth: '900px', width: '100%', position: 'relative', zIndex: 1 }}>
            ...
            <p style={{ fontSize: 'clamp(16px, 2vw, 19px)', color: 'rgba(250, 250, 250, 0.95)', marginBottom: theme.spacing['2xl'], lineHeight: 1.7, textShadow: '0 2px 12px rgba(0,0,0,0.6)' }}>
              Meat-free excellence — {venues.length} vegetarian restaurants
            </p>
          ...
          <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 16px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
              <div style={{ color: theme.colors.text.secondary, fontSize: '14px' }}>
                Showing <strong style={{ color: theme.colors.text.primary }}>{sorted.length}</strong> vegetarian restaurants
              </div>
...
export async function getStaticProps() {
  ...
  try {
    ...
    const venues = allVenues.filter(v => v && (v.cuisines?.some(c => c.toLowerCase().includes('vegetarian')) || v.dietaryTags?.some(t => t.toLowerCase().includes('vegetarian')) || v.name?.toLowerCase().includes('vegetarian'))).sort((a, b) => (b.rating || 0) - (a.rating || 0));
    return { props: { venues }};
  } catch (error) {
    return { props: { venues: [] } };
  }
