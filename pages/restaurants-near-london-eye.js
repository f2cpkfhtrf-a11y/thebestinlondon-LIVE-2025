...
export async function getStaticProps() {
  ...
  try {
    ...
    const landmarkVenues = allVenues
      .filter(v => v && (v.category === 'landmark' || (v.formatted_address && v.formatted_address.toLowerCase().includes('southbank'))))
      .map(v => {
        const address = v.vicinity || v.formatted_address || '';
        const areas = ['South Bank', 'Waterloo', 'Lambeth', 'Westminster', 'Southwark', 
                      'Borough', 'London Bridge', 'Bankside', 'Vauxhall'];
...
export default function LondonEyeRestaurants({ venues, lastUpdated }) {
  ...
  const schemaData = {
    ...
    "mainEntity": {
      ...
      "description": `${venues.length} carefully curated restaurants near London's iconic landmark`,
      ...
      "itemListElement": venues.slice(0, 20).map((venue, index) => ({
        ...
        "item": {
          ...
          "address": {
            ...
            "addressLocality": venue.area || "South Bank",
  ...
  const faqSchema = {
    ...
    "mainEntity": [
      {
        "@type": "Question",
        "name": "What are the best restaurants near the London Eye?",
        "acceptedAnswer": {
          "@type": "Answer",
          "text": `There are over ${venues.length} excellent restaurants within walking distance of the London Eye along the South Bank. Top-rated options include establishments on Southbank Centre, Waterloo, and Westminster Bridge Road.`
        }
      },
      {
        "@type": "Question",
        "name": "How much do restaurants near London Eye cost?",
        "acceptedAnswer": {
          "@type": "Answer",
          "text": "Restaurants near the London Eye range from £12-18 per person for casual dining to £50+ for fine dining with views. Most mid-range restaurants cost around £18-30 per person."
        }
      },
      {
        "@type": "Question",
        "name": "Are there restaurants with views of the London Eye?",
        "acceptedAnswer": {
          "@type": "Answer",
          "text": "Yes, many restaurants along the South Bank and across the Thames offer views of the London Eye. Rooftop restaurants and riverside venues provide the best vistas."
        }
      },
      {
        "@type": "Question",
        "name": "What cuisine types are available near London Eye?",
        "acceptedAnswer": {
          "@type": "Answer",
          "text": "The South Bank area offers diverse cuisine including British, Italian, Indian, Asian fusion, and international chains. There are options for all dietary requirements including vegetarian and vegan."
        }
      },
      {
        "@type": "Question",
        "name": "Do I need to book restaurants near London Eye in advance?",
        "acceptedAnswer": {
          "@type": "Answer",
          "text": "Booking is recommended for popular restaurants, especially on weekends and during tourist season. Many restaurants accept walk-ins during off-peak hours."
        }
      }
    ]
  };
  ...
  return (<>
    <Head>
      <title>Best Restaurants Near London Eye 2025 - {venues.length} Top Rated Options | The Best in London</title>
    ...
    <div style={{ minHeight: '100vh', background: '#fafafa', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif' }}>
      <div style={{ background: 'white', borderBottom: '1px solid #e5e7eb', padding: '12px 20px' }}>
        <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
          <nav style={{ fontSize: '14px', color: '#6b7280' }}>
            ...
            <span>Restaurants Near London Eye</span>
          </nav>
        </div>
      </div>

      <header style={{ background: 'linear-gradient(135deg, #3b82f6 0%, #2563eb 100%)', color: 'white', padding: '60px 20px 50px', boxShadow: '0 4px 6px rgba(0,0,0,0.1)' }}>
        <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
          ...
          <p style={{ fontSize: 'clamp(16px, 2.5vw, 20px)', opacity: 0.95, margin: '0 0 12px 0', lineHeight: '1.6', maxWidth: '900px' }}>
            {venues.length} expertly curated restaurants within walking distance of London's iconic landmark
          </p>
          <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap', fontSize: '14px', opacity: 0.9, marginTop: '16px' }}>
            <span>🎡 Near London Eye</span>
            <span>⭐ Real Google Ratings</span>
            <span>💰 Prices from £12-50+</span>
            <span>📍 South Bank Area</span>
          </div>
        </div>
      </header>
        ...
        <div style={{ background: 'white', borderRadius: '16px', padding: '24px', boxShadow: '0 10px 40px rgba(0,0,0,0.12)' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr auto auto', gap: '12px', marginBottom: '16px', alignItems: 'center' }}>
            ...
            <button onClick={() => setShowFilters(!showFilters)}
              style={{ padding: '14px 20px', fontSize: '16px', border: '2px solid #3b82f6', borderRadius: '10px', 
                      background: showFilters ? '#3b82f6' : 'white', color: showFilters ? 'white' : '#3b82f6', 
      ...
      <div style={{ maxWidth: '1400px', margin: '0 auto 24px', padding: '0 20px' }}>
        <p style={{ fontSize: '16px', color: '#6b7280', margin: 0 }}>
          Showing <strong style={{ color: '#3b82f6' }}>{filtered.length}</strong> of <strong>{venues.length}</strong> restaurants
        </p>
      </div>
        ...
        {filtered.length > 0 ? (
            ...
            {filtered.map((venue, idx) => {
              ...
              <article key={venue.place_id || idx} itemScope itemType="https://schema.org/Restaurant"
                ...
                <div style={{
                  ...
                  {venue.area && (
                    <div style={{ position: 'absolute', bottom: '16px', left: '16px', background: 'rgba(255,255,255,0.95)', color: '#2563eb',
                ...
                <div style={{ padding: '24px', flex: 1, display: 'flex', flexDirection: 'column' }}>
                  ...
                  {venue.priceEstimate && (
                    <div style={{ marginBottom: '18px', padding: '10px 12px', background: '#eff6ff', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      <span style={{ color: '#2563eb', fontWeight: '700', fontSize: '15px' }}>💰 {venue.priceEstimate} per person</span>
                      <span style={{ color: '#3b82f6', fontSize: '14px', fontWeight: '600' }}>{'£'.repeat(venue.price_level || 2)}</span>
                    </div>
                  )}

                  <a href={`/restaurant/${venue.name.toLowerCase().replace(/[^a-z0-9]+/g, '-')}-${venue.place_id.slice(-8)}`}
                    style={{ background: 'linear-gradient(135deg, #3b82f6 0%, #2563eb 100%)', color: 'white', padding: '14px 24px', border: 'none', borderRadius: '12px',
                            fontSize: '15px', fontWeight: '600', cursor: 'pointer', width: '100%', transition: 'all 0.2s', boxShadow: '0 2px 8px rgba(59, 130, 246, 0.3)',
                            display: 'block', textAlign: 'center', textDecoration: 'none' }}
                    onMouseEnter={(e) => { e.currentTarget.style.transform = 'scale(1.02)'; e.currentTarget.style.boxShadow = '0 4px 12px rgba(59, 130, 246, 0.4)'; }}
          ...
          <div style={{ textAlign: 'center', padding: '80px 20px', background: 'white', borderRadius: '20px', boxShadow: '0 2px 12px rgba(0,0,0,0.08)' }}>
            ...
            <button onClick={() => { setSearchTerm(''); setFilterArea('all'); setFilterPrice('all'); setFilterRating('all'); }}
              style={{ background: '#3b82f6', color: 'white', padding: '12px 24px', border: 'none', borderRadius: '10px', fontSize: '15px', fontWeight: '600', cursor: 'pointer' }}>
              Clear All Filters
            </button>
          </div>
        )}
      ...
      <section style={{ maxWidth: '1400px', margin: '0 auto 60px', padding: '0 20px' }}>
        <div style={{ background: 'white', borderRadius: '20px', padding: '40px', boxShadow: '0 2px 12px rgba(0,0,0,0.08)' }}>
          ...
          <div style={{ maxWidth: '900px', margin: '0 auto' }}>
            {[
              { q: "What are the best restaurants near the London Eye?", a: `There are over ${venues.length} excellent restaurants within walking distance of the London Eye along the South Bank. Top-rated options include establishments on Southbank Centre, Waterloo, and Westminster Bridge Road.` },
              { q: "How much do restaurants near London Eye cost?", a: "Restaurants near the London Eye range from £12-18 per person for casual dining to £50+ for fine dining with views. Most mid-range restaurants cost around £18-30 per person." },
              { q: "Are there restaurants with views of the London Eye?", a: "Yes, many restaurants along the South Bank and across the Thames offer views of the London Eye. Rooftop restaurants and riverside venues provide the best vistas." },
              { q: "What cuisine types are available near London Eye?", a: "The South Bank area offers diverse cuisine including British, Italian, Indian, Asian fusion, and international chains. There are options for all dietary requirements including vegetarian and vegan." },
              { q: "Do I need to book restaurants near London Eye in advance?", a: "Booking is recommended for popular restaurants, especially on weekends and during tourist season. Many restaurants accept walk-ins during off-peak hours." }
            ].map((faq, idx) => (
