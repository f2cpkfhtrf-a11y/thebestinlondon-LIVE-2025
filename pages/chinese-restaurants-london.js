    ...
    <>
      ...
      <div style={{ minHeight: '100vh', background: theme.colors.bg.primary, color: theme.colors.text.primary }}>
        ...
        <section style={{
          ...
          background: `linear-gradient(to bottom, rgba(11,11,11,0.5), rgba(11,11,11,0.75)), url('https://images.unsplash.com/photo-1525755662778-989d0524087e?w=2400&q=90')`,
          ...
          <div style={{ maxWidth: '900px', width: '100%', position: 'relative', zIndex: 1 }}>
            ...
            <p style={{ fontSize: 'clamp(16px, 2vw, 19px)', color: 'rgba(250, 250, 250, 0.95)', marginBottom: theme.spacing['2xl'], lineHeight: 1.7, textShadow: '0 2px 12px rgba(0,0,0,0.6)' }}>
              From authentic dim sum to Szechuan spice — {venues.length} exceptional Chinese restaurants
            </p>
            <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: '12px' }}>
              ...
              <span style={{
                ...
                fontSize: '14px',
                fontWeight: 600
              }}>🥟 Dim Sum • 🍜 Noodles • 🥠 Authentic</span>
            </div>
          </div>
        </section>
          ...
          <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 16px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
              <div style={{ color: theme.colors.text.secondary, fontSize: '14px' }}>
                Showing <strong style={{ color: theme.colors.text.primary }}>{sorted.length}</strong> Chinese restaurants
              </div>
                  ...
                  <article style={{ background: theme.colors.bg.elevated, border: `1px solid ${theme.colors.border.subtle}`, borderRadius: '16px', overflow: 'hidden', transition: 'all 0.3s ease', cursor: 'pointer', height: '100%' }} className="venue-card">
                    ...
                    <div style={{ padding: 'clamp(16px, 3vw, 20px)' }}>
                      ...
                      <div style={{ display: 'flex', gap: '12px', marginBottom: '12px' }}>
                        <span style={{ color: theme.colors.text.secondary, fontSize: '14px' }}>🥟 Chinese</span>
...
export async function getStaticProps() {
  ...
  try {
    ...
    const venues = allVenues.filter(v => v && (v.cuisines?.some(c => c.toLowerCase().includes('chinese')) || v.name?.toLowerCase().includes('chinese') || v.name?.toLowerCase().includes('dim sum'))).sort((a, b) => (b.rating || 0) - (a.rating || 0));
    return { props: { venues }};
  } catch (error) {
    return { props: { venues: [] } };
  }
