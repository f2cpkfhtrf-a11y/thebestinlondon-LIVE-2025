...
export default function AreaTile({ slug, name, count = 0, className = '' }: AreaTileProps) {
  // Construct the image path with fallback
  const imageSrc = `/images/heroes/areas/${slug}.webp`;
  const fallbackSrc = '/images/heroes/areas-default.webp';
  ...
  return (
    ...
    >
      <div className="relative aspect-[4/3] overflow-hidden">
        <Image
          src={imageSrc}
          alt={`${name} dining in London`}
          fill
          sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw"
          className="object-cover transition-transform duration-500 group-hover:scale-105"
          loading="lazy"
          onError={(e) => {
            // Fallback to default area image if the specific one fails
